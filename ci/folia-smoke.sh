...
SERVER_KIND=folia
if ! curl -fsSL --retry 8 --retry-all-errors --retry-delay 20 "https://fill.papermc.io/v3/projects/folia/versions/${MC}/builds" -o builds.json 2>/dev/null \
   || ! python3 -c 'import json,sys; b=json.load(open("builds.json")); sys.exit(0 if isinstance(b, list) and b else 1)'; then
  SERVER_KIND=paper
  curl -fsSL --retry 8 --retry-all-errors --retry-delay 20 "https://fill.papermc.io/v3/projects/paper/versions/${MC}/builds" -o builds.json
fi
