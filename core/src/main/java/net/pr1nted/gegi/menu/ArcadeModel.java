...
import net.pr1nted.gegi.catalog.ShelfLoads;
...
public final class ArcadeModel {
    ...
    public ArcadeModel(Catalog catalog) {
        ...
        for (Catalog.SiteLink site : catalog.sites()) {
            siteEntries.add(new GameEntry(site.title(), site.url(), site.url().getHost(), Optional.<URI>empty(), "",
                    Strings.string("gegi.site.browse"), Optional.<String>empty()));
        }
    }
    ...
    public String tabLabel(int index) {
        if (index == 0) return Strings.string("gegi.tab.recommended");
        if (index == tabCount() - 1) return Strings.string("gegi.tab.sites");
    ...
    public Optional<GameEntry> takePasted() {
        ...
        if (!uri.isPresent()) {
            notice = Strings.string("gegi.link.invalid");
    ...
    public String footer() {
        return notice != null ? notice : Strings.string("gegi.opens_in_browser");
    }
    ...
    public String status(List<GameEntry> games) {
        ...
        if (load != null && !load.done) return Strings.string("gegi.loading");
        if (load != null && !load.error.isEmpty()) return Strings.string("gegi.failed", load.error);
        if (games.isEmpty()) return Strings.string("gegi.empty");
        return null;
    }
    ...
    public String metaLine(GameEntry game) {
        ...
        for (String part : new String[]{featured(game) ? Strings.string("gegi.featured") : "", game.site(), game.price()}) {
