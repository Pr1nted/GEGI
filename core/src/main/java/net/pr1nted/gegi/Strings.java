...
/**
 * GEGI's strings, read straight from its own jar.
 *
 ...
 * on, so a translatable key showed up raw ("gegi.title") there. Reading the
 * lang file from the classpath works the same on every loader and every version.
 */
public final class Strings {
    private Strings() {}

    public static final String RESOURCE = "/assets/gegi/lang/en_us.json";
    ...
    private static Map<String, String> load() {
        ...
        try (InputStream in = Strings.class.getResourceAsStream(RESOURCE)) {
            ...
            Log.error("GEGI could not read its strings", e);
        }
        return strings;
    }
