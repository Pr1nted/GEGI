...
/**
 * The arcade, drawn with 1.20.6's GUI. What it shows and what a click does is
...
public final class ArcadeScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float a) {
        ...
        graphics.drawCenteredString(this.font, this.title, this.width / 2, 9, GOLD);
        ...
        for (int i = 0; i < games.size(); i++) {
            ...
            graphics.drawString(this.font, this.font.plainSubstrByWidth(game.title(), textWidth), textX, rowY + 6, featured ? GOLD : WHITE);
            graphics.drawString(this.font, this.font.plainSubstrByWidth(game.blurb(), textWidth), textX, rowY + 20, DIM);
            graphics.drawString(this.font, this.font.plainSubstrByWidth(model.metaLine(game), textWidth), textX, rowY + 34, featured ? GOLD : DIM);
        }
        graphics.disableScissor();

        String status = model.status(games);
        if (status != null) {
            graphics.drawCenteredString(this.font, status, this.width / 2, LIST_TOP + (games.isEmpty() ? 20 : -10 + (bottom - LIST_TOP)), DIM);
        }
        graphics.drawString(this.font, this.font.plainSubstrByWidth(model.footer(), right - DONE_WIDTH - 8 - left),
                left, this.height - FOOTER + 12, model.footerIsNotice() ? GOLD : DIM);
        framesDrawn++;
    }

    private void drawThumbnail(GuiGraphics graphics, GameEntry game, int x, int y) {
        ...
        if (ready.isPresent()) {
            ...
            graphics.blit(r.id(), x, y, THUMB_WIDTH, THUMB_HEIGHT, 0, 0,
