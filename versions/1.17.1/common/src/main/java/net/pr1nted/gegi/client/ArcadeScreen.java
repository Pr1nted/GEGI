...
/**
 * The arcade, drawn with 1.17.1's GUI. What it shows and what a click does is
...
public final class ArcadeScreen extends Screen {
    ...
    protected void init() {
        ...
        for (int i = 0; i < model.tabCount(); i++) {
            final int index = i;
            Button button = this.addRenderableWidget(new Button(model.tabX(this.width, i), TABS_Y, model.tabWidth(this.width), 20, new TextComponent(model.tabLabel(i)), b -> {
        ...
        search.setSuggestion(search.getValue().isEmpty() ? Lang.text("gegi.search").withStyle(s -> s.withColor(TextColor.fromRgb(DIM & 0xFFFFFF))).getString() : "");
        ...
        link.setSuggestion(link.getValue().isEmpty() ? Lang.text("gegi.link").withStyle(s -> s.withColor(TextColor.fromRgb(DIM & 0xFFFFFF))).getString() : "");
        ...
        this.addRenderableWidget(new Button(right - OPEN_LINK_WIDTH, FIELDS_Y - 1, OPEN_LINK_WIDTH, 20, Lang.text("gegi.link.open"), b -> model.takePasted().ifPresent(game -> ArcadeClient.play(this, game))));

        this.addRenderableWidget(new Button(right - DONE_WIDTH, this.height - FOOTER + 6, DONE_WIDTH, 20, CommonComponents.GUI_DONE, b -> this.onClose()));
    }
