...
final class DevScreenshot {
    ...
    private static void shoot(Minecraft minecraft, String target, String detail, boolean thenStop) {
        try (NativeImage shot = Screenshot.takeScreenshot(minecraft.getWindow().getWidth(), minecraft.getWindow().getHeight(), minecraft.getMainRenderTarget())) {
            shot.writeToFile(java.nio.file.Paths.get(target));
