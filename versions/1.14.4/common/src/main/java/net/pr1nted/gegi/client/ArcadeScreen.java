...
public final class ArcadeScreen extends Screen {
    ...
    protected void init() {
        ...
        EditBox search = new EditBox(this.font, left, FIELDS_Y, half, 18, Lang.string("gegi.search"));
        search.setSuggestion(search.getValue().isEmpty() ? Lang.string("gegi.search") : "");
        ...
        EditBox link = new EditBox(this.font, left + half + ROW_GAP, FIELDS_Y, half - OPEN_LINK_WIDTH - ROW_GAP, 18, Lang.string("gegi.link"));
        link.setMaxLength(512);
        link.setSuggestion(link.getValue().isEmpty() ? Lang.string("gegi.link") : "");
        ...
        this.addButton(link);
        this.addButton(new Button(right - OPEN_LINK_WIDTH, FIELDS_Y - 1, OPEN_LINK_WIDTH, 20, Lang.string("gegi.link.open"), b -> model.takePasted().ifPresent(game -> ArcadeClient.play(this, game))));
