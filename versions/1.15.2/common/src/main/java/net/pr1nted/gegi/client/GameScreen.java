...
public final class GameScreen extends Screen {
    ...
    private final BrowserRuntime.FrameTarget target = new BrowserRuntime.FrameTarget() {
        ...
        public ByteBuffer prepare(int width, int height) {
            ...
            return MemoryUtil.memByteBuffer(((NativeImageAccessor) (Object) texture.getPixels()).gegi$pixels(), width * height * 4);
        }
    ...
    @Override
    protected void init() {
        this.addButton(new Button(2, 1, 60, 20, Lang.string("gegi.game.back"), b -> this.onClose()));
        this.addButton(new Button(this.width - 112, 1, 110, 20, Lang.string("gegi.game.browser"), b -> {
    ...
    public void render(int mouseX, int mouseY, float a) {
        ...
        if (texture != null) {
            this.minecraft.getTextureManager().bind(TEXTURE_ID);
            blit(0, BAR, this.width, this.height - BAR, 0, 0,
                    textureWidth, textureHeight, textureWidth, textureHeight);
        } else {
            fill(0, BAR, this.width, this.height, 0xFF030806);
            drawCenteredString(this.font, statusLine(), this.width / 2, this.height / 2, 0xFF8FB9A2);
        }
        fill(0, 0, this.width, BAR, 0xE0030806);
        super.render(mouseX, mouseY, a);
        String heading = runtime.title().isEmpty() ? game.title() : runtime.title();
        int room = this.width - 64 - 116 - 8;
        drawString(this.font, this.font.substrByWidth(heading, Math.max(20, room)), 68, 7, 0xFFE8FFF1);
        String hint = Lang.string("gegi.game.leave_hint");
        if (this.font.width(heading) + this.font.width(hint) + 12 < room) {
            drawString(this.font, hint, this.width - 116 - this.font.width(hint), 7, 0xFF8FB9A2);
        }
    }
        ...
        switch (runtime.state()) {
            case DOWNLOADING:
                return runtime.progress() >= 0
                        ? Lang.string("gegi.game.downloading", runtime.progress())
                        : Lang.string("gegi.game.unpacking");
            case FAILED:
                return Lang.string("gegi.game.failed", runtime.error());
            default:
                return Lang.string("gegi.game.starting");
        }
    }
