...
final class Thumbnails {
    ...
    Optional<Ready> get(URI uri) {
        ...
        Http.get(uri).thenApply(Images::toPng).whenComplete((png, error) -> {
            ...
            Minecraft.getInstance().addScheduledTask(() -> upload(uri, png));
        });
    ...
    private void upload(URI uri, byte[] png) {
        ...
        try {
            ...
            Minecraft.getInstance().getTextureManager().loadTexture(id, new DynamicTexture(image));
    ...
    void releaseAll() {
        ...
        for (Optional<Ready> ready : textures.values()) {
            ready.ifPresent(r -> Minecraft.getInstance().getTextureManager().deleteTexture(r.id()));
        }
        textures.clear();
    }
}
