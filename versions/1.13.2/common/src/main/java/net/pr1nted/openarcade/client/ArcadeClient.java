...
/**
 * The client side of Open Arcade on Minecraft 1.13.2 (Forge). The Forge module calls
 * {@link #screen(GuiScreen)} for its "config" button, and adds the Options button,
 * the command and the tick hook through Forge's events (Forge 25 ships no Mixin).
 */
public final class ArcadeClient {
    ...
    public static final Set<String> COMMANDS = Collections.unmodifiableSet(new HashSet<>(Arrays.asList("arcade", "openarcade")));
    ...
    public static void open(GuiScreen parent) {
        Minecraft.getInstance().displayGuiScreen(screen(parent));
    }

    /** The Open Arcade button for an Options screen, added by the Forge event and remembered for the self-test. */
    public static GuiButton optionsButton(GuiScreen options, int x, int y) {
        optionsButton = Buttons.of(x, y, 100, 20, Lang.string("openarcade.button"), () -> open(options));
    ...
    public static void play(GuiScreen parent, GameEntry game) {
        ...
        }, Lang.string("openarcade.consent.title"),
                Lang.string("openarcade.consent.message", BrowserRuntime.dataDir()),
                Lang.string("openarcade.consent.download"),
                Lang.string("openarcade.consent.browser"), 0));
    }
