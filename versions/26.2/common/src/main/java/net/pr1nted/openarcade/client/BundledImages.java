...
final class BundledImages {
    ...
    private static Optional<Thumbnails.Ready> upload(String location) {
        ...
        try (InputStream in = BundledImages.class.getResourceAsStream(resource)) {
            ...
            Identifier id = Identifier.fromNamespaceAndPath(Constants.MOD_ID, "bundled/" + name);
            Minecraft.getInstance().getTextureManager().register(id, new DynamicTexture(() -> "Open Arcade " + location, image));
